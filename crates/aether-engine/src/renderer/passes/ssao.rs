...
impl Pass for SSAOPass {
    ...
    /// signatures are read, so the half-resolution AO target is sized correctly).
...
mod tests {
    ...
    fn init_creates_resources() {
        ...
        let _pass = SSAOPass::new(&device);
    }

    #[test]
    fn set_screen_size_updates_ao_write_size() {
        let Some((device, _queue)) = headless_device_queue() else {
            eprintln!("SKIP: no GPU adapter available");
            return;
        };
        let mut pass = SSAOPass::new(&device);
